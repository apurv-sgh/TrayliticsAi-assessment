...
app.get('/api/health', (req, res) => res.json({ ok: true, service: 'matchwise-api' }))
...
connectDatabase().then(() => httpServer.listen(port, () => console.log(`Matchwise API listening on ${port}`))).catch((error) => { console.error('Database startup failed', error); process.exit(1) })
